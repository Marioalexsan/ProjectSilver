...
namespace Game
{
    Medkit::Medkit() : Actor(nullptr), healthToGrant(0), pickUpDelay(30)
    {
    ...
    void Medkit::Update()
    {
        ...
        if (player != nullptr && pickUpDelay == 0 && !IsDestructionSignalled() &&
            (player->GetTransform().position - transform.position).Length() < 40.0)
        {
            auto& stats = ((Actor*)player)->GetStatsReference();
            stats.maxHealth = Utility::ClampValue(stats.maxHealth + healthToGrant, 0.0, 100.0);
            stats.health = Utility::ClampValue(stats.health + int(1.5 * healthToGrant),
                                               0.0,
                                               stats.maxHealth);
