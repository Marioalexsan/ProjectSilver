...
namespace Game {
    ...
    void ShadowAI::Update() {
        ...
        if (action == Actions::Recovery && entity->GetComponent().GetCurrentAnimationID() == "Shadow_AxeIdle") {
            action = Actions::MeleeChase;
        }

        int pistolCost = 1450;

        if (pistolUseCooldown == 0 && pistolCredits >= pistolCost && entity->GetComponent().GetCurrentAnimationID() == "Shadow_AxeIdle") {
            ...
            pistolShotCount = 3 + (currentDifficulty >= 3 ? 1 : 0) + (currentDifficulty >= 6 ? 1 : 0) + (currentDifficulty >= 10 ? 1 : 0) + (currentDifficulty >= 13 ? 1 : 0);
            nextShot = counter + Utility::ClampValue(50 - currentDifficulty * 2, 15, 60);
        }
        else if (pistolShotCount == 0 && entity->GetComponent().GetCurrentAnimationID() == "Shadow_PistolIdle") {
            action = Actions::MeleeChase;
            entity->GetComponent().SetDefaultAnimation("Shadow_AxeIdle");
        ...
        Vector2 moveTargetVector = player->GetTransform().position - this->entity->GetTransform().position + (player->GetTransform().position - lastFramePlayerPos) * movePredictionStrength * 60.0;
        double moveTargetDirection = moveTargetVector.Angle();
        Vector2 trueTargetVector = player->GetTransform().position - this->entity->GetTransform().position;
        double trueTargetDirection = trueTargetVector.Angle();
        double distance = moveTargetVector.Length();
        double angleDelta = abs(Math::GetAngleDifference(entity->GetTransform().direction, moveTargetDirection));
        ...
        if (entity->GetComponent().GetCurrentAnimationID() == "Shadow_AxeSwing" && turnStrength > maxSwingStrength) {
        ...
        case Actions::MeleeChase: {
            ...
            if (distance >= 400 && distance <= 800) {
                ...
                forwardStrength *= 1.3 - 0.3 * Utility::ClampValue((800.0 - distance) / 400.0, 0.0, 1.0);
            }
            if (distance < 400) {
                strafeStrength = 3.5;
            }
        ...
        case Actions::PistolRetaliation: {
            if (distance > 600) {
                forwardStrength *= 1.0 + 0.3 * Utility::ClampValue((distance - 600) / 200.0, 0.0, 1.0);
            }
            else if (distance > 450) {
                forwardStrength *= 1.0 - Utility::ClampValue((600 - distance) / 150.0, 0.0, 1.0);
                strafeStrength *= 1.1;
            }
            else {
                forwardStrength *= -0.6 * Utility::ClampValue((450 - distance) / 450.0, 0.0, 1.0);
                strafeStrength *= 1.3;
            }
        } break;
        }
        ...
        case Actions::MeleeChase: {
            if (entity->GetComponent().GetCurrentAnimationID() == "Shadow_AxeIdle" && postSwingDelay <= 0) {
                switch (rand() % 2) {
                case 0: action = Actions::MeleeLungeHitCombo; break;
                case 1: action = Actions::MeleeTurnHitCombo; break;
                }
                swingCount = 1 + (currentDifficulty >= 3 ? 1 : 0) + (currentDifficulty >= 7 ? 1 : 0) + (currentDifficulty >= 14 ? 1 : 0);
            }
        } break;
        ...
        case Actions::MeleeTurnHitCombo: {
            ...
            if (swingCount > 0) {
                if (entity->GetComponent().GetCurrentAnimationID() == "Shadow_AxeIdle" && distance < distanceToUse && angleDelta < angleDeltaToUse) {
        ...
        if (entity->GetComponent().GetCurrentAnimationID() == "Shadow_PistolIdle" && pistolShotCount > 0) {
            ...
            if (!playerOutOfPistolRange && counter >= nextShot) {
                ...
                hitRecently = false;
                GenericWeaponFireLogic(15.0 + Globals::Difficulty(), 4.0 - 0.25 * currentDifficulty);
    ...
    void ShadowAI::GenericWeaponFireLogic(double damageToDeal, double angleDeltaToApply) {
        ...
        if (results.size() > 1) {
            ...
            for (auto& elem : results) {
                ...
                if (elem.second->GetCombatLayer() == Collider::CombatLayer::Players || elem.second->GetColliderType() == Collider::ColliderType::Static) {
                    hit = elem.second;
                    distance = elem.first;
                    break;
                }
