...
namespace Game {
	namespace CollisionMaster {
		...
		std::pair<bool, Vector2> CheckCollision(const BoxCollider& A, const BoxCollider& B, bool checkOnly) {
		...
		std::pair<bool, Vector2> CheckCollision(const BoxCollider& A, const SphereCollider& B, bool checkOnly) {
			auto APos = A.GetPosition();
			auto BPos = B.GetPosition();
			auto AWH = A.GetSize();
			auto ALeft = APos.x;
			...
			if (dist < BRad) {
				...
				// Manipulate closest# values to create push std::vector
		...
		std::pair<bool, Vector2> CheckCollision(const SphereCollider& A, const SphereCollider& B, bool checkOnly) {
		...
		std::pair<bool, Vector2> CheckCollision(const SphereCollider& A, const BoxCollider& B, bool checkOnly) {
			// Just calls the other version and reverses push std::vector
			auto var = CheckCollision(B, A);
			return { var.first, var.second * -1 };
		}

		std::pair<bool, double> RaycastVSBox(const Vector2& A, const Vector2& B, const BoxCollider& Box) {
		...
		std::pair<bool, double> RaycastVSSphere(const Vector2& A, const Vector2& B, const SphereCollider& Sphere) {
