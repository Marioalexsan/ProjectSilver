...
namespace Game {
    ...
    void LogHandler::LogInfoToCrashLog(std::ofstream& errorLog) {
        if (errorLog.good()) {
            ...
            errorLog << "Resolution: " << ConfigHandler::RetrieveItem("resolution") << std::endl;
            errorLog << "Window Mode: " << ConfigHandler::RetrieveItem("videomode") << std::endl;
        }
    }
	...
	void LogHandler::Log(std::string message, MessageType type) {
        ...
        if (type == MessageType::Error) {
            ...
            if (!errorLog) {
                    std::string fatalError = "LogHandler was unable to log an Error. The error message was: " + message;
