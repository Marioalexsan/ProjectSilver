...
namespace Game
{
    ...
    void PlayerPseudoAI::Update()
    {
        ...
        static const std::vector<std::string> switchAnimations = {"Player_TakeOutRifle",
        ...
        {
            return std::find(uninterruptibleAnimations.begin(),
                             uninterruptibleAnimations.end(),
                             animation) != uninterruptibleAnimations.end();
        };

        auto IsWeaponSwitch = [&](const std::string& animation)
        {
            return std::find(switchAnimations.begin(), switchAnimations.end(), animation) !=
                   switchAnimations.end();
        };
        ...
        if (stats.health <= 0.0 || stats.maxHealth <= 20.0)
        {
            ...
            {
                lowHPVignette.SetAlpha(
                    vignetteAlpha + uint8_t(ceil((255 - vignetteAlpha) * 0.04)));
            }
            heartbeatTime = 60;
        ...
        else
        {
            ...
            if (shieldRegenCounter >= 120)
            {
                ...
                {
                    stats.shieldHealth = Utility::ClampValue(stats.shieldHealth += healing,
                                                             0.0,
                                                             stats.maxShieldHealth);
            ...
            if (inShield ||
                stats.shieldHealth < stats.maxShieldHealth && shieldRegenCounter >= 0)
            {
                shieldHealth.SetAlpha(255);
                shieldHealth.SetText("Shield: " + std::to_string(int(stats.shieldHealth)) +
                                     " / " + std::to_string(int(stats.maxShieldHealth)));
            ...
            playerHealth.SetText("Health: " + std::to_string(int(stats.health)) + " / " +
                                 std::to_string(int(stats.maxHealth)));
            ...
            stamina.SetText("Boost: " + std::to_string(int(stats.stamina / 30.0)) +
                            " / " + std::to_string(int(stats.maxStamina / 30.0)));
        ...
        if (equippedWeapon == 0)
        {
            gunAmmo.SetText("Ammo: " + std::to_string(currentPistolAmmo) + " / " +
                            std::to_string(maxPistolAmmo));
        ...
        {
            gunAmmo.SetText(
                "Ammo: " + std::to_string(currentRifleAmmo) + " / " +
                std::to_string(maxRifleAmmo) + " (" + std::to_string(rifleAmmoPool) +
                ")" + (currentRifleAmmo + rifleAmmoPool >= 60 ? " (Max!)" : ""));
        ...
        if (wasReloadingPistol == true &&
            entity->GetComponent().GetCurrentAnimationID() == "Player_PistolIdle")
        {
            currentPistolAmmo  = maxPistolAmmo;
            wasReloadingPistol = false;
        }
        if (wasReloadingRifle == true &&
            entity->GetComponent().GetCurrentAnimationID() == "Player_RifleIdle")
        {
        ...
        if (playerAnimation == "Player_PistolShoot" ||
            playerAnimation == "Player_RifleShoot")
        {
        ...
        if (playerAnimation == "Player_PistolReload" ||
            playerAnimation == "Player_RifleReload")
        {
            moveSpeed /= 3.0;
        }
        else if (inShield)
        ...
        // Override with Joystick Input if needed
        sf::Vector2f axis = game.Input.GetLeftAxis();
        if (axis.length() > Deadzone)
        {
            horizontalDir = axis.x;
            verticalDir   = axis.y;
        }

        // This smooths out the player's movement
        Vector2 targetSpeed       = Vector2(0.0, 0.0);
        Vector2 targetSpeedNormal = Vector2::NormalVector(
            Vector2(horizontalDir, verticalDir).Angle());
        ...
        if (boostVector != Vector2::Zero)
        {
            ...
            {
                Globals::Game()
                    .CreateAfterImageEffect(entity->GetTransform(),
                                            entity->GetComponent().GetCurrentAnimationID(),
                                            entity->GetComponent().GetFrame());
            }
            boostVector *= 0.8;
        ...
        bool shieldInput = Globals::Game().Input.IsButtonDown(ButtonCode::Right) ||
                           Globals::Game().Input.IsJoystickDown(
                               InputHandler::JoystickCode::RightBumper);

        // Shielding logic
        if (shieldRegenCounter >= 0 && shieldInput &&
            (!IsUninterruptible(playerAnimation) ||
             playerAnimation == "Player_ShieldDown" && hasPerfectGuarded) &&
            !IsWeaponSwitch(playerAnimation))
        {
            wasReloadingPistol  = false;
            wasReloadingRifle   = false;
            inShield            = true;
        ...
        if ((!shieldInput || shieldRegenCounter < 0) &&
            playerAnimation == "Player_ShieldWalk")
        {
        ...
        bool shootInput = Globals::Game().Input.IsButtonDown(ButtonCode::Left) ||
                          Globals::Game().Input.GetRightTrigger() > Deadzone;
        ...
        if (shootInput)
        {
            ...
            else if (equippedWeapon == 1 && (playerAnimation == "Player_RifleIdle" ||
                                             playerAnimation == "Player_RifleShoot" &&
                                                 entity->GetComponent().GetFrame() == 4))
            {
                if (currentRifleAmmo > 0)
                {
                    // Max 8 degree spread
        ...
        if (playerAnimation != "Player_ShieldWalk" &&
            playerAnimation != "Player_ShieldUp" && playerAnimation != "Player_ShieldDown")
        {
            inShield          = false;
            hasPerfectGuarded = false;
        }
        ...
        if (wasInWeaponSwitch)
        {
            ...
            bool switchCondition = component.GetFrame() == component.GetFrameCount() - 1 &&
                                   component.GetAccumulatedUpdates() >=
                                       component.GetUpdatesPerFrame() - 2;
            if (targetWeaponEquip == 0 &&
                (playerAnimation == "Player_PistolIdle" ||
                 playerAnimation == "Player_PutAwayRifle" && switchCondition))
            {
                entity->GetComponent().SwitchAnimation("Player_TakeOutPistol");
                equippedWeapon    = 0;
                wasInWeaponSwitch = false;
            }
            if (targetWeaponEquip == 1 &&
                (playerAnimation == "Player_RifleIdle" ||
                 playerAnimation == "Player_PutAwayPistol" && switchCondition))
            {
        ...
        bool pistolSwitchInput = game.Input.IsKeyPressedThisFrame(KeyCode::NumOne) ||
                                 game.Input.IsJoystickPressedThisFrame(
                                     InputHandler::JoystickCode::X);
        bool rifleSwitchInput = game.Input.IsKeyPressedThisFrame(KeyCode::NumTwo) ||
                                game.Input.IsJoystickPressedThisFrame(
                                    InputHandler::JoystickCode::Y);

        // Weapon Switch Logic - Put Away Weapon
        if (pistolSwitchInput && equippedWeapon != 0)
        {
        ...
        if (rifleSwitchInput && equippedWeapon != 1)
        {
        ...
        bool reloadInput = game.Input.IsKeyPressedThisFrame(KeyCode::R) ||
                           game.Input.IsJoystickPressedThisFrame(InputHandler::JoystickCode::A);
        ...
        if (reloadInput)
        {
            ...
            else if (playerAnimation == "Player_RifleIdle" &&
                     currentRifleAmmo < maxRifleAmmo && rifleAmmoPool > 0)
            {
        ...
        // Makes the player face the mouse position or aim with joystick

        double aimAngle = 0;

        std::pair<int, int> var = game.Input.GetMousePosition();

        if (std::abs(lastMousePos.x - var.first) >= 5 ||
            std::abs(lastMousePos.y - var.second) >= 5)
        {
            // Switch to mouse aiming
            useJoystickAiming = false;
        }
        else if (game.Input.GetRightAxis().length() > Deadzone)
        {
            // Switch to joystick aiming
            useJoystickAiming = true;
        }

        lastMousePos = {var.first, var.second};

        if (!useJoystickAiming)
        {
        ...
        {
            auto vec = game.Input.GetRightAxis();
    ...
    void PlayerPseudoAI::OnHitByAttack(Actor* attacker, double damage)
    {
        if (entity != nullptr)
        {
            ...
            if (inShield)
            {
                auto attackAngle = (attacker->GetTransform().position -
    ...
    void PlayerPseudoAI::GenericWeaponFireLogic(double damageToDeal, double angleSpread)
    {
        ...
        if (results.size() > 1)
        {
            ...
            for (auto& elem : results)
            {
                ...
                if (elem.second->GetCombatLayer() == Collider::CombatLayer::Enemies ||
