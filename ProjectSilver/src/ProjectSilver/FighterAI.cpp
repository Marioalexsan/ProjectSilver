...
namespace Game
{
    ...
    void FighterAI::Update()
    {
        ...
        if (distance < 400.0)
        {
            forwardStrength = Utility::ClampValue(400.0 - distance, 0.0, 200.0) / 200.0 *
                              (-2.4 - 0.8 * Globals::Difficulty());
            strafeStrength = 3.3;
        }
        ...
        Vector2 shootVector = targetVector +
                              (player->GetTransform().position - lastFramePlayerPos) *
                                  predictionStrengthToUse * 60.0;
        ...
        if (shouldAim)
        {
            if (!seesPlayer)
            {
                nextShot += (int)((counter + aimTime - nextShot) *
                            (0.6 - 0.15 * Globals::Difficulty()));
            }
            else
            {
            }
        ...
        if (counter >= nextShot)
        {
            ...
            uint64_t bulletID = Globals::Game()
                                    .AddEntity(EntityType::FighterBulletProjectile,
                                               entity->GetTransform().position +
                                                   Vector2::NormalVector(
                                                       entity->GetTransform().direction) *
                                                       40);
            Entity* theBullet = Globals::Game().GetEntity(bulletID);
            theBullet->GetTransform()
                .direction = Utility::ScrollValue(entityDirection +
                                                      (shootDirection - entityDirection) * 0.06,
                                                  0.0,
                                                  360.0);
