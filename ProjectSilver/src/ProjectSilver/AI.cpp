...
namespace Game
{
    AI::AI(Actor* owner) : smoothSpeed(0.0, 0.0), counter(-45), entity(owner)
    {
    }

    AI::AI() : smoothSpeed(0.0, 0.0), counter(-45), entity(nullptr)
    {
    }
    ...
    bool AI::DelayedSpawningLogic()
    {
        ...
        if (counter <= 0)
        {
            entity->GetComponent().SetAlpha(
                uint8_t(Utility::ClampValue(45 + counter, 0, 45) * 255 / 45));
            return true;
        }
        return false;
    }
    ...
    bool AI::HasPlayerLineOfSight()
    {
        ...
        auto results = Globals::Game().CreateRayCastHitList(entity->GetCollider().GetPosition(),
                                                            ((Actor*)Globals::ThePlayer())
                                                                ->GetCollider()
                                                                .GetPosition());
