...
std::map<std::string, std::string> ConfigHandler::configPairs;
	...
	void ConfigHandler::WritePair(std::ofstream& file, const std::string& key, const std::string& value) {
		if (file) {
			int start = (int)key.find_first_not_of(" \n\r\t");
			std::string trimmedKey = key.substr(start, key.find_last_not_of(" \n\r\t") - start + 1);
			start = (int)value.find_first_not_of(" \n\r\t");
			std::string trimmedValue = value.substr(start, value.find_last_not_of(" \n\r\t") - start + 1);
