...
namespace Game
{
    Actor::Actor(AI* brain) : baseStats(), Entity()
    {
        this->brain = brain;
        if (brain != nullptr)
        {
    ...
    {
        Move(knockback * Utility::ClampValue(baseStats.knockbackResistance, 0.0, 100.0) *
             (1.0 / 60.0));
    }
    ...
    {
        GetTransform().direction = Utility::ScrollValue(GetTransform().direction + amount,
                                                        0.0,
                                                        360.0);
    }
